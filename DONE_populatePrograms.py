...
def populate_programs_from_csv():
    conn = get_db_connection()
    cursor = conn.cursor()

    try:
        with open('iastate_graduate_programs.csv', 'r', encoding='utf-8') as file:
            ...
            for row in csv_reader:
                ...
                interdepartmental = 0
                if len(row) > 1 and row[1].strip().lower() == 'interdepartmental':
                    interdepartmental = 1

                
                insert_query = "INSERT INTO Program (name, interdepartmental) VALUES (%s, %s)"
                
                # 4. Using a list instead of a single-item tuple for stability
                cursor.execute(insert_query, (program_name, interdepartmental))
                
        # Commit inside the try block so it only saves if reading succeeded
        conn.commit()
        print("Programs populated successfully!")
